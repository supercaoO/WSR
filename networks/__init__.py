...
def define_net(opt):
    ...
    elif which_model.find('WSR') >= 0:
        from .wsr_arch import WSR
        net = WSR(in_channels=opt['in_channels'], out_channels=opt['out_channels'], num_features=opt['num_features'],
                  num_simdb=opt['num_groups'], upscale_factor=opt['scale'])
